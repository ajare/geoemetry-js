...
export class Mesh {
  ...
  moveVertex(id, position) { const v = this.vertices.get(id); if (!v) throw new GeometryError(`Vertex ${id} does not exist`); v.position = Vector2.from(position); for (const e of v.edges) this.edges.get(e)?.update(this); for (const p of this.polygons.values()) if (p.vertexIndices().includes(id) || p.holes.some(h => this.polygons.get(h)?.usesVertex(id))) p.invalidate(); this._emit('vertexMoved', { id, vertex: v }); return this; }
